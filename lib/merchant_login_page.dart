...
class _MerchantLoginPageState extends State<MerchantLoginPage> {
  ...
  Future<void> _signIn() async {
    ...
    try {
      ...
      // Step 2: Perform the Role Check for Merchants
      final merchantRef =
          FirebaseDatabase.instance.ref('merchants/${user.uid}');
      final snapshot = await merchantRef.get();

      if (snapshot.exists && mounted) {
        // Step 3: If check passes, navigate to the merchant home page
        Navigator.of(context).pushReplacement(
            MaterialPageRoute(builder: (context) => const MerchantHomePage()));
      } else {
        // Step 4: If check fails, the user is not a merchant. Sign them out and show an error.
        await FirebaseAuth.instance.signOut();
        _showFeedback("You are not registered as a merchant.");
      }
  ...
  Future<void> _forgotPassword() async {
    ...
    try {
      ...
      if (!event.snapshot.exists) {
        _showFeedback("This email is not registered as a merchant.");
        if (mounted) {
          setState(() {
            _isLoading = false;
          });
